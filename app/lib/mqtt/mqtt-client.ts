...
import { onData } from "../wind/WindConsumer"
...
export const connectMqtt: (() => Promise<() => void>) = async (config = getAppConfig()) => {
    const instance = await createMqttInstance(config.mqtt)
    client = instance.client

    await subscribe(client, `${config.storm.windTopic}/#`)
    online()
    log.info("MQTT subscription active")

    client.on("message", async (topic, message) => {
        log.debug(`MQTT message received on ${topic}`, message)
        onData(message)
    })
